  ...
  return (
    <div style={{ minHeight: "100vh", background: S.bg, display: "flex", flexDirection: "column" }}>
      ...
      <div style={{ maxWidth: 600, margin: "0 auto", width: "100%", padding: mob ? "28px 16px" : "48px 24px" }}>
        ...
        <div style={{ marginBottom: 28 }}><span style={labelStyle}>{root === "space" ? "장소 분류" : root === "scene" ? "장면 분류" : "물건 분류"}</span><div style={{ display: "flex", gap: 16, flexWrap: "wrap", marginTop: 8 }}>{subCats.map(c => { const val = root === "space" ? cat : root === "scene" ? type : otype; const setter = root === "space" ? setCat : root === "scene" ? setType : setOtype; return <button key={c} onClick={() => setter(val === c ? "" : c)} style={catBtn(val === c)}>{c}</button>; })}</div></div>
        <div style={{ marginBottom: 28 }}><span style={labelStyle}>제목</span><input value={title} onChange={e => setTitle(e.target.value)} placeholder="기록의 제목" style={inputStyle} /></div>
