...
export default function Sloist(){
  ...
  const [keepBold,sKeepBold]=useState(false);
  ...
  const keep=useCallback(async(id)=>{if(!auth.user){pendingAction.current={type:"keep",id};goTo("login");return;}const was=savedIds.includes(id);
    ...
    sKeepBold(true);setTimeout(()=>sKeepBold(false),1200);
    const{error}=was
      ...
      <div style={{flex:"1 0 auto"}}>
        ...
        <div style={{maxWidth:640,margin:"0 auto",padding:mob?"0 24px":"0 32px"}}>
          ...
          <div style={{marginTop:mob?36:56,paddingTop:mob?14:18,borderTop:"1px solid "+S.ln,display:"flex",alignItems:"center",gap:mob?14:20}}>
            <button onClick={()=>keep(dl.id)} style={{fontFamily:S.ui,fontSize:mob?12:12,fontWeight:keepBold?500:300,letterSpacing:"0.08em",color:isSaved(dl.id)?S.ac:S.txF,background:"none",border:"none",cursor:"pointer",padding:mob?"6px 0":"4px 0",transition:"font-weight 1.2s ease, color .6s"}}>{isSaved(dl.id)?"보관됨":"보관"}</button>
